...
def get_info_velocity(info, receive, color, id):
    receive = receive
    obstacles = receive.get_infos(color, id)
    return obstacles
...
class APF():
    ...
    def __init__(self, s_x, s_y, g_x, g_y, info, receive, k_att=3, k_rep=8000, rr=100,
                 step_size=10, max_iters=500, goal_threshold=10, att_threshold=100, dis_threshold=30, color='blue', id='5'):
        """
        :param s_x, s_y: 起点
        :param g_x, g_y: 终点
        ...
        obstacles_velocity = get_info_velocity(info, receive, color,id)
        self.barrierInfo = np.array(obstacles)
        self.dis_threshold = dis_threshold
        self.obstacles = [Vector2d(OB[0], OB[1]) for OB in obstacles]
        self.obstacles_velocity = [Vector2d(OB[0], OB[1]) for OB in obstacles_velocity]
        self.k_att = k_att
        self.k_rep = k_rep
        self.rr = rr  # 斥力作用范围
    ...
    def get_velocity(self, color, robot_id):
        self.receive.get_info(color, robot_id)
        return self.receive.robot_info['vx'], self.receive.robot_info['vy']

    def get_orientation(self, color, robot_id):
        self.receive.get_info(color, robot_id)
        return self.receive.robot_info['ori']
    ...
    def repulsion(self):
        ...
        for i in range(len(self.obstacles)):
            ...
            ori_RO = math.atan2(obstacle.deltaY-self.current_pos.deltaY, obstacle.deltaX-self.current_pos.deltaX)
            ori_RG = math.atan2(self.goal.deltaY-self.current_pos.deltaY, self.goal.deltaX-self.current_pos.deltaX)
            alpha = ori_RO-ori_RG

            obstacle_velocity_now = self.obstacles_velocity[i]
            vx, vy = self.get_velocity(self.color, self.id)
            robot_velocity_now = Vector2d(vx, vy)
            vor = obstacle_velocity_now-robot_velocity_now
            gamma = math.atan2(vor.deltaY, vor.deltaX)-ori_RO

            x1 = (self.vomax+self.vrmax)/self.Dthreatmin
            y1 = (self.vomax+self.vrmax)/(math.pi*self.Dthreatmin)
            normalization = 1/(self.k1*x1 + self.k2*y1+self.k3)
