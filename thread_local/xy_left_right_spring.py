    ...
    def line_control(self, now_x, now_y, now_ori, path, i, N, target_x, target_y, infos=None, color='blue', robot_id=4, threshold=30, index=1):
        ...
        for y in range(M):
            if barriers[y][2] == color and barriers[y][3] == robot_id:
                ...
                break
        for t in range(M-1):
            road = distance(point_now, [barriers[t][0], barriers[t][1]])
            if road < 20:
                if target_x > 0:
                    return (-self.x * cos(now_ori) - self.y * sin(now_ori)) / (road*road), (
                    self.x * sin(now_ori) - self.y * cos(now_ori)) / (road*road), False  # 往右上角
                else:
                    return (self.x * cos(now_ori) + self.y * sin(now_ori)) / (road*road), (
                    -self.x * sin(now_ori) + self.y * cos(now_ori)) / (road*road), False  # 往左下角
                # print('here')
                # if now_x>=0 and now_y>=0:
                #     return (-self.x * cos(now_ori) - self.y * sin(now_ori))/road, (self.x * sin(now_ori) - self.y * cos(
                #         now_ori))/road, False
